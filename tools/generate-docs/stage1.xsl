...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:xd2="http://www.oecd.org/ns/schema/xdocs" xmlns:xd0="http://www.oecd.org/ns/schema/xdocs"
  ...
  <xsl:template match="xs:import">
    ...
    <xsl:if test="not(contains($importedSchema, concat('[', resolve-uri(@schemaLocation, base-uri()), ']')))">
      ...
      <saxon:assign name="importedSchema"
        select="concat($importedSchema, '[', resolve-uri(@schemaLocation, base-uri()), ']')"/>
  ...
  <xsl:template match="xs:complexType | xs:simpleType">
    ...
    <xsl:variable name="thisName">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of
            select="concat(parent::xs:element/@name | parent::xs:attribute/@name, '-', generate-id())"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
            ...
            <xsl:otherwise>
              <xsl:choose>
                <xsl:when test="$redefinitions//*[name() = $elementName and @name = $thisName]/xs:annotation">
                  <xsl:apply-templates
                    select="$redefinitions//*[name() = $elementName and @name = $thisName]/xs:annotation"/>
                </xsl:when>
                <xsl:otherwise>
          ...
          <xsl:if test="$redefinitions != ''">
            <xsl:apply-templates
              select="$redefinitions//*[name() = $elementName and @name = $thisName]/xs:complexContent/xs:extension/*">
              <xsl:with-param name="nsPrefix" select="$nsPrefix"/>
            </xsl:apply-templates>
          </xsl:if>
        </xsl:otherwise>
