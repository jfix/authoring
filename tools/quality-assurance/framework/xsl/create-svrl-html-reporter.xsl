...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc><xd:desc> Set to true to use the role values as classes </xd:desc></xd:doc>
    <xsl:param name="role-as-style" select="'true'"/>

    <xd:doc><xd:desc> Set to true to use the role values as labels </xd:desc></xd:doc>
    <xsl:param name="role-as-label" select="'true'"/>

    <xd:doc><xd:desc> Set to true to use the role values as titles </xd:desc></xd:doc>
    <xsl:param name="role-as-title" select="'true'"/>
        
    <xd:doc><xd:desc> Set to true to use the id values as labels </xd:desc></xd:doc>
    <xsl:param name="id-as-label" select="'true'"/>
    
    <xd:doc><xd:desc> Set to true to use the id values as titles </xd:desc></xd:doc>
	...
	<xd:doc>
		<xd:desc><xd:p>Match the root svrl element and use it to generate
		the skeleton of our output stylesheet.</xd:p></xd:desc>
	</xd:doc>
	...
	<xd:doc>
		<xd:desc><xd:p>Give an active-pattern element, create the calls to the templates we are going
		to generate for each assert or report. This template builds up the structure of the output
		document using the groups created above. Within the pattern we group all the fired-rules 
		for output.</xd:p></xd:desc>
	</xd:doc>
	...
	<xd:doc>
		<xd:desc><xd:p>If a fired rule has associated assertion failures or rule successes, then
		they will be in the group with the rule. Output the group and the template applications
		into the div for that group. Do nothing if the group only contains the rule.</xd:p></xd:desc>
	</xd:doc>
	...
	<xd:doc>
		<xd:desc><xd:p>Generate the xsl:apply-templates we will use for each assert
		or report. Uses the location from the SVRL to generate the apply templates
		which is handled by the generic handler in svr-</xd:p></xd:desc>
	</xd:doc>
	...
	<xd:doc><xd:desc>Generate a namespace node from node and a prefix</xd:desc></xd:doc>
	...
	<xsl:template match="svrl:text" mode="generate-params">
		<axsl:with-param name="message" select="."/>
	</xsl:template>
